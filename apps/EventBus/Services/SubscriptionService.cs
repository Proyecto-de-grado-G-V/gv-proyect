    ...
    public async Task AddSubscriptionAsync(string eventType, string callbackUrl)
    {
        ...
        if (exists)
        {
            _logger.LogInformation(
                $"Subscription for event '{eventType}' with callback URL '{callbackUrl}' already exists."
            );
            return;
        }

        var subscription = new Subscription { EventType = eventType, CallbackUrl = callbackUrl };
    ...
    public async Task<ICollection<string>> ForwardEventAsync(EventWrapper eventWrapper)
    {
        ...
        foreach (var url in relevantSubscribers)
        {
            ...
            try
            {
                ...
                {
                    _logger.LogError(
                        $"Failed to send event to {url}. Status code: {response.StatusCode}"
                    );
                }
            }
            catch (Exception ex)
